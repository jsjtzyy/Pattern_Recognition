...
class rbm_layer():
    '''init rbm layer'''
    def __init__(self, input_size = 784, hidden_nodes = 500, x = None):
        if x == None:
...
def train(rbm1, rbm2, batch_size=100):
    '''here we build a multi-layer neural net from the result of two rbm layers'''
    x_placeholder = tf.placeholder(tf.float32, shape=[None, 784])
    y_placeholder = tf.placeholder(tf.float32, shape=[None, 10])
    w1 = tf.Variable(rbm1.old_w)
    b1 = tf.Variable(rbm1.old_hb)
    w2 = tf.Variable(rbm2.old_w)
    b2 = tf.Variable(rbm2.old_hb)
    y1 = tf.nn.sigmoid(tf.matmul(x_placeholder, w1) + b1)
    y2 = tf.nn.sigmoid(tf.matmul(y1, w2) + b2)

    w = tf.Variable(tf.random_normal([200, 10]))
    b = tf.Variable(tf.zeros([10]))
    y_hat = tf.nn.softmax(tf.matmul(y2, w) + b)
    '''define error function and training process'''
    ...
    '''the following code are for confusion matrix'''
...
if __name__ == "__main__":
    rbm0 = rbm_layer()
    rbm1 = rbm_layer(input_size=500, hidden_nodes=200)
    print "unsupervised train layer0"
    for i in range(1):
        rbm0.unsupervised_train()
    print "upsupervised train layer1"
    trX = logistic(np.matmul(trX, rbm0.old_w) + rbm0.old_hb)
    print type(trX)
    for i in range(1):
        rbm1.unsupervised_train()
