...
pca_trX = pca.fit_transform(trX)
pca_teX = pca.transform(teX)

print "pca done"

def train():
    x_placeholder = tf.placeholder(tf.float32, shape=[None, 200])
    y_placeholder = tf.placeholder(tf.float32, shape=[None, 10])  # Define a placeholder for the label

    w = tf.Variable(tf.random_normal([200, 10]))  # Random initialize the weight and bias
    ...
    for step in range(0, 100):
        for index in range(55000 / batch_size - 1):
            batch_x = pca_trX[index: index + batch_size]
        ...
        if step % 1 == 0:
            ...
            print(sess.run(accuracy, feed_dict={x_placeholder: pca_trX, y_placeholder: trY}))
            print("test  accuracy")
            print(sess.run(accuracy, feed_dict={x_placeholder: pca_teX, y_placeholder: teY}))

    pred_trY = sess.run(y_hat, feed_dict={x_placeholder: pca_trX, y_placeholder: trY})
    pred_teY = sess.run(y_hat, feed_dict={x_placeholder: pca_teX, y_placeholder: teY})
