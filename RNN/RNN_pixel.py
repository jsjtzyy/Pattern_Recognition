...
class RNN():
    def __init__(self):
        ...
        self.batch_size = 100
        self.T = 49              # time sequence for downsampled image
        self.input_size = 1      # input dimension
        self.hidden_nodes = 100  # number of hidden nodes
        ...
        # define basic cell in RNN network
        self.time_delay_cell = tf.nn.rnn_cell.BasicRNNCell(self.hidden_nodes)
        init_state = self.time_delay_cell.zero_state(self.batch_size, dtype=tf.float32)
        ...
        self.pred = tf.matmul(self.unpacked_out[-1], self.weights['W_out']) + self.bs['b_out']
        ...
        self.sess.run(tf.initialize_all_variables())  # initialization
    ...
    def train(self, epochs):
        ...
        np.savetxt("RNN_pixel.txt", accuracy_result)
